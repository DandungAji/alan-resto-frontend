    ...
    <div class="print:hidden">
      <div class="bg-white rounded-sm shadow-sm p-5 flex flex-col" style="height: 85vh;">
        <div class="flex items-center mb-5 flex-shrink-0">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-gray-600 mr-3" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clip-rule="evenodd" /></svg>
            <h2 class="text-xl font-bold text-gray-800">Pesanan</h2>
        </div>
        <div class="flex-grow overflow-y-auto pr-2">
            <div v-if="cart.items.length === 0" class="text-center text-gray-500 my-10">Keranjang masih kosong</div>
            <div v-else v-for="item in cart.items" :key="item.id" class="flex items-center mb-4">
                ...
                <div class="flex-grow"><p class="font-semibold text-gray-700">{{ item.name }}</p></div>
        ...
        <div class="mt-4 pt-4 flex-shrink-0">
            <button @click="cart.clearCart()" class="w-full text-red-600 font-semibold py-2 px-4 rounded-sm border-2 border-red-500 hover:bg-red-50 transition-colors mb-4">Clear Cart</button>
            <div class="w-full flex justify-between space-x-2 mb-4">
              <button @click="handleSaveBill" class="w-full bg-green-500 text-white font-semibold py-2 px-4 rounded-sm hover:bg-green-600 transition-colors">Save Bill</button>
              <button @click="handlePrintBill" class="w-full bg-green-500 text-white font-semibold py-2 px-4 rounded-sm hover:bg-green-600 transition-colors">Print Bill</button>
            </div>
