...
function handlePay() {
  ...
  alert(`Pembayaran berhasil!\nTotal: ${formatPrice(totalBill.value)}\nBayar: ${formatPrice(paymentAmount.value)}\nKembali: ${formatPrice(change.value)}`);
...
<template>
  <div class="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50">
    ...
    <div class="bg-white p-6 md:p-8 rounded-lg shadow-xl w-full max-w-4xl gap-8">
    ...
    <div class="flex flex-col md:flex-row">  
          ...
          <table class="w-full text-left text-sm">
            <thead class="bg-gray-100">
              <tr>
                ...
                <th class="p-3 font-semibold text-gray-600 text-right">Harga</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="(item, index) in cart.items" :key="item.id">
                <td class="p-3 text-gray-700">{{ index + 1 }}</td>
                <td class="p-3 text-gray-700">{{ item.name }} x{{ item.quantity }}</td>
                <td class="p-3">
                  <img :src="item.photo" :alt="item.name" class="h-10 w-12 object-cover rounded-md">
                </td>
                <td class="p-3 text-gray-800 font-medium text-right">{{ formatPrice(item.price * item.quantity) }}</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <div class="ml-8 w-full md:w-2/5 flex flex-col justify-center">
        <div class="space-y-5">
          <div>
            <label for="payment-amount" class="text-center block mb-2 text-sm font-medium text-gray-700">Uang Pembeli (Rp)</label>
            <input 
          ...
          <div class="flex justify-end gap-4">
            <button @click="emit('close')" class="w-full px-6 py-2 border border-gray-300 rounded-sm text-gray-700 hover:bg-gray-50 transition-colors">
              Close
            </button>
            <button @click="handlePay" class="w-full px-8 py-2 bg-sky-500 text-white font-semibold rounded-sm hover:bg-sky-600 transition-colors">
              Pay!
            </button>
          </div>
